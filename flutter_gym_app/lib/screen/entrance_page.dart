...
class _EntrancePageState extends State<EntrancePage> {
  ...
  void _doEntrance() async{
    final String uid = Text(uidController.text).data;

    http.Response response = await http.post(
        Constants.SERVER_ADDRESS + '/api/entrance', body: jsonEncode(
        <String, String>{'uid': uid}));
    if (response.statusCode == 200) {
      Navigator.push(context, MaterialPageRoute(builder: (context) => EntrancePage()));
      //todo add allert for confirm
    } else {
  ...
  Future _scan() async {
    ...
    this.uidController.text = barcode;
  }
}